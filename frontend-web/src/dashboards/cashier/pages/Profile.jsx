    ...
    <MainLayout>
      <div className="p-8 max-w-[1000px]">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <p className="text-xs text-[#aaa] font-mono mb-1">Account</p>
            <h1 className="text-2xl font-bold text-[#111]">My Profile</h1>
          </div>
          <button
            onClick={() => setEditing(!editing)}
            className="flex items-center gap-2 px-4 py-2 border border-[#ddd] bg-white rounded-lg text-sm text-[#555] hover:border-[#aaa] transition-colors"
          >
        ...
        {/* Hero card */}
        <div className="bg-[#111] text-white rounded-xl p-6 mb-6 flex items-center justify-between">
          <div className="flex items-center gap-5">
            <div className="relative">
              ...
              <div className="absolute bottom-0 right-0 w-5 h-5 rounded-full bg-[#22c55e] border-2 border-[#111] flex items-center justify-center">
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          ...
          <Card className="p-6">
            <h2 className="font-semibold text-[#111] mb-5">Personal Information</h2>
          ...
          {/* Security */}
          <Card className="p-6">
            <h2 className="font-semibold text-[#111] mb-5">Security & Authentication</h2>
            <div className="space-y-4">
              <div>
                <p className="text-[10px] text-[#aaa] font-mono uppercase tracking-widest mb-2">Two-Factor Authentication</p>
                <div className="flex items-center gap-3">
                  <Toggle checked={twoFA} onChange={setTwoFA} />
                  <span className="text-sm text-[#555]">Email OTP enabled</span>
                </div>
              </div>
              <div className="pt-2 border-t border-[#f0ede8]">
                ...
                <button className="px-4 py-2 bg-[#111] text-white text-sm rounded-lg hover:bg-[#222] transition-colors font-medium">
                  Update Password
                </button>
              </div>
              <div className="pt-3 border-t border-[#f0ede8]">
                <p className="text-xs text-[#888] font-mono">Last login: 22 Mar 2026, 08:14 AM</p>
                <p className="text-xs text-[#aaa] font-mono">Device: Chrome · Windows 11 · 103.22.18.x</p>
              </div>
            </div>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          {/* Shift History */}
          <Card className="p-6">
            <h2 className="font-semibold text-[#111] mb-4">Recent Shift History</h2>
            <div className="space-y-3">
              {shiftHistory.map((shift, i) => (
                <div key={i} className="flex items-center gap-3">
                  <span className="text-xs font-mono text-[#aaa] w-12">{shift.id}</span>
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-[#111]">{shift.date}</span>
                      {shift.status && (
                    ...
                    <p className="text-xs text-[#aaa] font-mono">{shift.hours}</p>
                  </div>
                  <span className="text-xs font-mono text-[#888]">{shift.txns} txns</span>
                </div>
              ))}
            </div>
          </Card>
          ...
          {/* Recent Activity */}
          <Card className="p-6">
            <h2 className="font-semibold text-[#111] mb-4">Recent Activity</h2>
            <div className="space-y-3">
              {recentActivity.map((item, i) => (
                <div key={i} className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <p className="text-sm font-medium text-[#111]">{item.action}</p>
                    <p className={`text-xs font-mono mt-0.5 ${item.isLink ? 'text-[#22c55e]' : 'text-[#aaa]'}`}>
                      {item.detail}
                    </p>
                  </div>
                  <span className="text-xs text-[#aaa] font-mono shrink-0">{item.time}</span>
                </div>
              ))}
            </div>
          </Card>
